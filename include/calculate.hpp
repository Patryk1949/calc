...
void plot(const std::vector<FuncRunInfo> &functions, RunInfo &runInfo, bool costumStep);
...
constexpr double ssss = axisPrecission(21.0);


void plot(const std::vector<FuncRunInfo> &functions, RunInfo &runInfo, const bool costumSteps){
	...
	float axisPosX = sp::clamp(0.0, -first * scaleX, 600.0);
	float axisPosY = sp::clamp(0.0, top * scaleY, 600.0);
	...
	
	{
		...
		sf::Font font;
		font.loadFromFile("/home/basher/Programming/Programs/unispace.ttf");
		Draw:{
